...
const CourseField = ({ onSubmitHandler, initialValues }) => {
	const state = useSelector(state => {
		return {
			isLoading: state.admin.serviceChangeLoading,
			message: state.admin.serviceChangeMessage
		}
	});
	if(!initialValues){
		initialValues = {
			            ...
			            >
			                {({values, handleSubmit}) => (
			                     <form onSubmit={handleSubmit} className="registerForm">
			                        <Input type="text" name="title" label="Имя услуги"/>
			                        <Input type="text" name="description" label="Описания"/>
			                        <Input type="text" name="price" label="Цена"/>
